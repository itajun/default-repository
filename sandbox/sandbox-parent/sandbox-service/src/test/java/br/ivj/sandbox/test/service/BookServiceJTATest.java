...
public class BookServiceJTATest {
	...
	public void expectedBookCreateTest() {
		...
		try {
			...
			fail("Should have thrown an exception due to the title.");
		...
		// Let's try to migrate the same book and hope the transaction is rolled
		// back
		try {
			bookService.migrateBook(ID_FAIL_TO_MIGRATE);
			fail("Should have thrown an exception due to unique constraint.");
		...
		assertNotNull("Should have returned the newly create book.",
				migratedBook);
